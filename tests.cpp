...
std::vector<uint8_t> readPPM(const std::string &filename) {
  /**
  Reads a PPM (Portable Pixmap) image file and returns its content as a vector
 of bytes.
...
TEST_CASE("generators test") {
  ...
  SUBCASE("mandelbrot_generator test") {
    ...
    are the following: scaling_factor = 1.0 center_real = 0.0 center_im = 0.0
    ...
    */
    ...
    test_mandelbrot_generation.mandelbrot_generator(scaling_factor, center_real,
                                                    center_im);
  ...
  SUBCASE("julia_generator test") {
    ...
    */
    ...
    const std::string referenceImageFile =
        "./TEST_IMAGES/0.300000_-0.450000.ppm";
...
TEST_CASE("mandelbrot_multiple_images test") {
  ...
  const double center_real = -0.74;
  const double center_im = 0.0;

  Mandelbrot test_M_m_i(dim);
  test_M_m_i.mandelbrot_multiple_images(end_scaling_factor, scaling_factor_step,
...
TEST_CASE("generate_julia_set test") {
  ...
  SUBCASE("Test Generated Files Exist") {
    ...
    CHECK(std::filesystem::exists(std::filesystem::path("JULIA") /
                                  "0.100000_-0.100000.ppm"));
    CHECK(std::filesystem::exists(std::filesystem::path("JULIA") /
                                  "0.200000_-0.200000.ppm"));
    CHECK(std::filesystem::exists(std::filesystem::path("JULIA") /
                                  "0.300000_-0.300000.ppm"));
    CHECK(std::filesystem::exists(std::filesystem::path("JULIA") /
                                  "0.400000_-0.400000.ppm"));
  }
