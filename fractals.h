...
int num_iter(std::complex<double> z0, std::complex<double> c, int max_iter,
             double thresh = 4) {
    /*
    Given a point on the complex plane a given constant c calculates the orbit given by the recursive formula z = z**2 + c of that point
    z0: starting point of the orbit
    c: complex constant
    thresh: arbitrary threshold to calculate if the orbit diverges

    returns the number of iterations before it diverges or reaches a given number of iters
   */
...
private:
  ...
  std::vector<double> board; //vector of the pixels of the images, rapresents the coordinates of the Argand Gauss plane on which the orbits are calculated
...
{
  /*
    Modifies inplace the board vector applying the recursive formula to assign a numerical value (color) to each coordinate in the complex plane
    z_real_bound: boundries of the image on the real axis
    z_im_bound: boundries of the image on the imaginary axis
    center_real: center of the image on real axis (translation of 0 in the real axis)
    center_im: center of the image on imaginary axis (translation of 0 in the imaginary axis)
    c: complex constant to generate julia set
...
class Mandelbrot : public Fractals {
  // class that inherits from Fractals for creating and visualizing the  mandelbrot set 
private:
  //everytime a Mandelbrot object is instatiated smkdir is called on a MANDELBROT named string -> it creates a directory to store the data in
  std::string data_dir;
public:
  ...
  std::complex<double> boundries(const double &scaling_factor) {
    /*
      ...
      returns a complex number, the boundries of the images are the real and imaginary parts of this number
    */
  ...
  void mandelbrot_multiple_images(const int &end_scaling_factor, const double &step, const double &zoom_center_real, const double &zoom_center_im) {
    /*
      generates multiple images of the mandebrot set by calling the generate mandelbrot function
      end_scaling_factor: last scaling factor before stopping zoom
      zoom_center_real: where the image is centered on the real axis 
      zoom_center_im: where the image is centered on the imaginary axis
    */
...
class Julia : public Fractals {
    // class that inherits from Fractals for creating and visualizing the  julia sets
private:
  //everytime a Mandelbrot object is instatiated smkdir is called on a MANDELBROT named string -> it creates a directory to store the data in
  std::string data_dir;
public:
